...
QUnit.test("valeur < 0 ", function(assert)
		{
			assert.expect(1);
			assert.throws(function(assert) {var m=new money(-1,"EUR")}, moneyInfZero, "Money inférieur à Zéro");

		}
		);

QUnit.test("currency > 3 caractère Exception ", function(assert)
		{
			assert.expect(1);
			assert.throws(function(assert) {var m=new money(10,"EURO")}, CurrencySupTrois, "Currency est supérieur à Trois");
